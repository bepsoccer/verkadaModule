function Remove-VerkadaAccessUserProfilePicture{
	...
	process {
		...
		$url = "https://vcerberus.command.verkada.com/user/photos/$org_id/$userId"

		try {
			Invoke-VerkadaCommandCall $url $org_id $body -x_verkada_token $x_verkada_token -x_verkada_auth $x_verkada_auth -usr $usr -Method 'DELETE'
			return "Profile pictured removed from user $userId"
		}
			...
			If ($err.StatusCode -eq 500){
				Write-Host "$($err.StatusCode) - $($err.message) - Most likely because no picture exists." -ForegroundColor Red
			} else {
